...
def train_baseline(dataloader, loss_fn):
    ...
    for q_vec, doc_vec, target in dataloader:
        pred = (nn.functional.cosine_similarity(q_vec, doc_vec) + 1) / 2
...
def test_baseline(dataloader, loss_fn):
    ...
    total_loss = 0
    for q_vec, doc_vec, target in dataloader:
        pred = cosine_similarity_from_0_to_1(doc_vec, q_vec)
        pred = pred.reshape([len(target), 1])
        loss = loss_fn(pred, target.to(torch.float32))
...
def cosine_similarity_from_0_to_1(doc_vec, q_vec):
    return (nn.functional.cosine_similarity(q_vec, doc_vec) + 1) / 2
